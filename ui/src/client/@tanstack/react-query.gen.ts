...
export const getCrawlerSitesQueryKey = (options?: Options<GetCrawlerSitesData>) => createQueryKey('getCrawlerSites', options);

/**
 * 获取已注册刮削器网站
 * 返回运行时已注册的刮削器网站列表, 供前端动态枚举使用
 */
export const getCrawlerSitesOptions = (options?: Options<GetCrawlerSitesData>) => {
    return queryOptions({
        queryFn: async ({ queryKey, signal }) => {
            const { data } = await getCrawlerSites({
                ...options,
                ...queryKey[0],
                signal,
                throwOnError: true
        ...
        queryKey: getCrawlerSitesQueryKey(options)
    });
};
